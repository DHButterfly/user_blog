    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.status == 0) {
                ...
                loadBlogTypeToNum(data);//渲染数据
                //再根据博客类型请求相应的博客数据，首次默认请求第一个,不嵌套的话就需要全局变量了
                getPreEndTypeToBlog(1,5,data[0].typeId);
            }
        },
...
function getPreEndTypeToBlog(pageNum,pageSize,typeId){
            ...
            if (res.status == 0) {
                ...
                createPageNav({
                    ...
                    afterFun : function(num){
                        getPreEndTypeToBlog(num,5,typeId);
                    }
                });
                loadTypeBlogList(list);//博客列表页面
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
...
function loadTypeBlogList(data){
    var blogList=data.list;
    $('#type-blog-list').empty();//清空
    // $("#totalBlog").val(blogList.length);
    for(let i=0;i<blogList.length;i++){
        $('#type-blog-list').append(
...
function loadBlogTypeToNum(data){
    //构造个颜色列表
    $('#typeAndNum').empty();
    var colorList=["orange","violet","purple","pink","green","teal","brown","DeepSkyBlue4","Bisque1","DeepPink","MedSpringGreen","Honeydew","PeachPuff","Cyan4","Pink4","Maroon4","grey31"];
    for(let i=0;i<data.length;i++){
        $('#typeAndNum').append(
            '<div class="ui labeled button m-margin-tb-tiny">' +
            '<a onclick="getPreEndTypeToBlog(1,5,'+data[i].typeId+')" class="ui '+colorList[i%data.length]+' button">'+data[i].typeName+'</a>' +
            '<div class= "ui inverted basic blue left pointing label">'+data[i].typeBlogNum+'</div>' +
            '</div>'
        )
    }
}
