    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.status == 0) {
                var data = res.data;
                console.log(data);
                $('#tagNum').text(data.length);
                loadBlogTagToNum(data);//渲染数据
                //再根据博客类型请求相应的博客数据，首次默认请求第一个,不嵌套的话就需要全局变量了
                getPreEndTagToBlog(1,5,data[0].tid);
            }
        },
...
function loadBlogTagToNum(data){
    $('#tagAndNum').empty();
    var colorList=["orange","violet","purple","pink","green","teal","brown","blue","yellow","grey","MedSpringGreen","Honeydew","PeachPuff","Cyan4","Pink4","Maroon4","grey31"];
    for(let i=0;i<data.length;i++){
        $('#tagAndNum').append(
            '<div class="ui labeled button m-margin-tb-tiny">' +
            '<a onclick="getPreEndTagToBlog(1,5,'+data[i].tid+')" class="ui '+colorList[i%data.length]+' button">'+data[i].tagName+'</a>' +
            '<div class= "ui inverted basic blue left pointing label">'+data[i].tagBlogNum+'</div>' +
            '</div>'
        );
    }
}
function getPreEndTagToBlog(pageNum,pageSize,tagId){
    $.ajax({
            ...
            if (res.status == 0) {
                ...
                createPageNav({
                    ...
                    afterFun : function(num){
                        getPreEndTagToBlog(num,5,tagId);
                    }
                });
                loadTagBlogList(list);//博客列表页面
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
...
function loadTagBlogList(data){
    var blogList=data.list;
    $('#tag-blog-list').empty();//清空
    // $("#totalBlog").val(blogList.length);
    for(let i=0;i<blogList.length;i++){
        var str='<div class="ui padded vertical segment m-padded-tb-large m-mobile-lr-clear">'+
        ...
        '</div>'+
        '</div>'+
        '</div><div class="right aligned five wide column">'+
        '<a href="#" target="_blank" class="ui teal basic label m-padded-tiny m-text-thin">'+blogList[i].blogType.name+'</a></div>'+
        '<div class="row"><div class="column">';
        for(let j=0;j<blogList[i].tags.length;j++){
            str+='<div class="ui basic teal left pointing label">'+blogList[i].tags[j]+'</div>';
        }
        str+='</div></div></div></div>'+
