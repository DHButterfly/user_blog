...
public class SessionUtil {
     ...
     * @return guser
     */
    public static BlogUser getUser(HttpSession session) {
        try {
            attribute = (SecurityContext) session.getAttribute("SPRING_SECURITY_CONTEXT");
            BlogUser principal = (BlogUser) attribute.getAuthentication().getPrincipal();
            return principal;
        } catch (Exception e) {
        }
        return null;
     ...
     * @param loginedUser
     */
    public static void deleteSameUser(HttpServletRequest request, SessionRegistry sessionRegistry, BlogUser loginedUser) {
        ...
        for (Object principal : o) {
            if (principal instanceof BlogUser && (loginedUser.getUsername().equals(((BlogUser) principal).getUsername()))) {
     ...
     * @param , SysMessageService sysMessageService
     */
    public static void dropPreviousUser2(HttpServletRequest request, SessionRegistry sessionRegistry, BlogUser loginedUser) {
     ...
     * @param sessionRegistry
     */
    public static void expireSession(HttpServletRequest request, BlogUser user, SessionRegistry sessionRegistry) {
        List<SessionInformation> sessionsInfo = null;
        if (null != user) {
            List<Object> o = sessionRegistry.getAllPrincipals();
            for (Object principal : o) {
                if (principal instanceof BlogUser && (user.getUsername().equals(((BlogUser) principal).getUsername()))) {
